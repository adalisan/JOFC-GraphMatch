...
n_vals<- c(seq(15,75,5),seq(80,190,10),seq(200,275,25))
...
npert<-length(pert)
  ...
  corr.results.list<- foreach(i=1:num.cores, .combine="c",.export=c("bitflip_MC_rep","run.experiment.JOFC")) %dopar% {
    ...
    corr.results.mc <- try(bitflip_MC_rep (pert,n,n_vals,embed.dim=12,
...
save.image(paste("JOFC-graph_bitflip_sim_100_",Sys.Date(),as.character(ceiling(runif(1)*1000)),"sp_highdim.Rdata"))