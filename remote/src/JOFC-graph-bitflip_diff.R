...
npert<-length(pert)
  ...
  corr.results.list<- foreach(i=1:num.cores, .combine="c",.export=c("bitflip_MC_rep","run.experiment.JOFC")) %dopar% {
    ...
    corr.results.mc <- try(bitflip_MC_rep (pert,n=n,n_vals,embed.dim=6,
			diss_measure="diffusion",it.per.G=iter_per_core,
...
save.image(paste("JOFC-graph_bitflip_diff_sim_300_",date(),".Rdata"))