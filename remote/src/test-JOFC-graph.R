...
run.experiment.JOFC<-function(G,Gp,n_vals,num_iter,embed.dim,diss_measure="default"){
  ...
  for (n_v_i in 1:length(n_vals)){
	...
	fname = paste("./logs/debug_JOFC_graph_func_run_experiment_JOFC",Sys.time(),floor(runif(n=1,max=100)) )
       save(list=c("corr.matches","jofc.result","M.result.1"),  file=fname)
  }
...
bitflip_MC_rep <- function (pert,n,n_vals,embed.dim,diss_measure){
  ...
  fname = paste("./cache/JOFC_graph",Sys.time(),floor(runif(n=1,max=100)))
  save( list=c("corr.match.array.mc"), file = fname)
