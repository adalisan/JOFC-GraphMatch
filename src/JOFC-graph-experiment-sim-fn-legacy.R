...
run.experiment.JOFC<-function(G,Gp,n_vals,num_iter,embed.dim,diss_measure="default",
                              graph.is.weighted=FALSE,graph.is.directed=FALSE,w.vals=w.vals,...){
...
bitflip_MC_rep <- function (pert,n,n_vals,embed.dim,diss_measure,it.per.G=1, 
  ...
  require(MCMCpack)
  require(clue)
  source("./lib/graph_embedding_fn_legacy.R")
...
enron_exp <- function (num_iter,n_vals_vec,embed.dim=2,weighted.graph=TRUE,
                       diss_measure="C_dice_weighted",symmetrize=TRUE,w.vals){
  #Not yet implemented
  ...
  new.n <- sum(!disc_v)
  n_vals_vec<- n_vals_vec[n_vals_vec<new.n]
  
  corr.matches<-run.experiment.JOFC(Ac_graph,Ag_graph,
...
enron_exp_par_sf_w <- function(num_iter,n_vals,embed.dim=3,weighted.graph=TRUE,
		diss_measure="C_dice_weighted",symmetrize=TRUE,w.vals, seq=FALSE) {
